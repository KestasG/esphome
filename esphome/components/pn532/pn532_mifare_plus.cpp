...
//  - NXP PN532 User Manual (Doc. 141520): Section 7.3.2 "InDataExchange" framing used by sendAPDU().
...
// EMV tag identifiers (EMV Book 3, Annex A; EMV Contactless Book C-2, Section 6).
constexpr uint8_t K_EMV_TAG_AID = 0x4F;
constexpr uint16_t K_EMV_TAG_PDOL = 0x9F38;
constexpr uint16_t K_EMV_TAG_COMMAND = 0x83;
constexpr uint16_t K_EMV_TAG_TRACK2 = 0x57;
constexpr uint16_t K_EMV_TAG_TRACK1 = 0x56;
constexpr uint16_t K_EMV_TAG_PAN = 0x5A;
constexpr uint16_t K_EMV_TAG_AFL = 0x94;
constexpr uint16_t K_EMV_TAG_FCI_TEMPLATE = 0x6F;
constexpr uint16_t K_EMV_TAG_FCI_PROPRIETARY_TEMPLATE = 0xA5;
constexpr uint16_t K_EMV_TAG_FCI_ISSUER_DISCRETIONARY = 0xBF0C;
constexpr uint16_t K_EMV_TAG_APPLICATION_TEMPLATE = 0x61;
constexpr uint16_t K_EMV_TAG_RESPONSE_TEMPLATE_FORMAT_2 = 0x77;
constexpr uint16_t K_EMV_TAG_RECORD_TEMPLATE = 0x70;
}  // namespace
...
static inline bool is_emv_template_tag_(uint16_t tag) {
  return tag == K_EMV_TAG_FCI_TEMPLATE || tag == K_EMV_TAG_FCI_PROPRIETARY_TEMPLATE ||
         tag == K_EMV_TAG_FCI_ISSUER_DISCRETIONARY || tag == K_EMV_TAG_APPLICATION_TEMPLATE ||
         tag == K_EMV_TAG_RESPONSE_TEMPLATE_FORMAT_2 || tag == K_EMV_TAG_RECORD_TEMPLATE;
}
...
bool PN532::read_mifare_plus_bytes_(uint8_t start_page, uint16_t num_bytes, std::vector<uint8_t> &data) {
  ...
  // EMV contactless flow (simplified for read-only use, cf. EMV Contactless Book C-2 Section 6):
  ...
  auto adf_name = find_tag_(apdu_response, K_EMV_TAG_AID);
  if (adf_name.empty()) {
  ...
  auto pdol = find_tag_(apdu_response, K_EMV_TAG_PDOL);
  ...
  command_apdu.push_back(K_EMV_TAG_COMMAND);
  ...
  if (auto track2_gpo = find_tag_(apdu_response, K_EMV_TAG_TRACK2); !track2_gpo.empty()) {
  ...
  auto afl = find_tag_(apdu_response, K_EMV_TAG_AFL);
  ...
  while (pos + 3 < afl.size()) {
    ...
    for (uint8_t record = first_record; record <= last_record; ++record) {
      ...
      if (sendAPDU(command_apdu, apdu_response)) {
        ...
        if (auto track2_record = find_tag_(apdu_response, K_EMV_TAG_TRACK2); !track2_record.empty()) {
          uint8_t pan_digits[19];
          size_t num_pan_digits = 0;
        ...
        if (auto track1_record = find_tag_(apdu_response, K_EMV_TAG_TRACK1); !track1_record.empty()) {
          uint8_t pan_digits[19];
          size_t num_pan_digits = 0;
        ...
        // ---- Try 5A (PAN, BCD) --------------------------------------------
        if (auto pan_tag = find_tag_(apdu_response, K_EMV_TAG_PAN); !pan_tag.empty()) {
          uint8_t pan_digits[19];
          size_t num_pan_digits = 0;
...
// Helper wrapping PN532 "InDataExchange" (PN532 User Manual, Doc 141520 Section 7.3.2 -
